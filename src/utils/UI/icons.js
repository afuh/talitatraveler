...
export const SocialIcon = ({ name, href, style, as }) => {
  const findIcon = (iconName) => {
    // capitalize the iconName
    const capitalizedName = iconName.replace(/\b\w/g, (l) => l.toUpperCase())

    const Component = require('react-icons/fa')['Fa' + capitalizedName]
    return <Component />
  }

  return (
    <SocialWrapper aria-label={name + ' Icon'} href={href} style={{ ...style }} as={as}>
      {findIcon(name)}
    </SocialWrapper>
  )
}
